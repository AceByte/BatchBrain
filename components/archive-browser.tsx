        ...
        <div className="grid">
          {filtered.map((card) => (
            <article key={card.key} className="card">
              <div className="card-head"><div className="card-title-group"><h3>{card.name}</h3>{card.is_batched ? <span className="batched">Batched</span> : null}</div><div className="card-actions-inline"><span className="tag">Archived {new Date(card.archived_at).toLocaleDateString()}</span><CocktailArchiveAction id={card.id} mode="restore" /></div></div>
